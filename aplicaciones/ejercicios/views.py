...
def lista_pruebas(request):
    ...
    pruebas_qs = Pruebas.objects.filter(usuario=usuario).order_by('id')

    # Añadimos la lógica de cálculo directamente a los objetos de la prueba
    for prueba in pruebas_qs:
        if prueba.cantidad_preguntas > 0:
            # Calculamos si el rendimiento es >= 70% y lo guardamos en un nuevo atributo
            prueba.es_aprobado = (prueba.correctas * 10 >= prueba.cantidad_preguntas * 7)
        else:
            prueba.es_aprobado = False

    return render(request, 'lista-pruebas.html', {'pruebas': pruebas_qs})
